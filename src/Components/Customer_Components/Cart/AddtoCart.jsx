...
const AddtoCart = ({ setFoodTokenlist, foodTokenlist, cartlist, setCartlist, setSideBarClicked, setCartClicked, setTokenClicked }) => {
  ...
  useEffect(() => {
    ...
    const cart = async () => {
      try {
        setLoading(true)
        const response = await fetch("https://foodtoken-backend-l2iv.onrender.com/cart", {
        method: "GET",
        headers: {
  ...
  const handleClick = async (id, foodId) => {
    try {
      const response = await fetch(`https://foodtoken-backend-l2iv.onrender.com/cart/delete/${id}`, {
        method: "DELETE",
        headers: {
          "x-auth-customertoken": localStorage.getItem("customertoken")
        }
  ...
  const increaseproduct = async (id, foodCount) => {
    try {
      ...
      const response = await fetch(`https://foodtoken-backend-l2iv.onrender.com/cart/edit/${id}`, {
        method: "PUT",
        body: JSON.stringify({
          foodCount: count
        }),
  ...
  const handleBuy = async (id, foodId) => {
    try {
      const buyresponse = await fetch(`https://foodtoken-backend-l2iv.onrender.com/token/add/${id}`, {
        method: "POST",
        body: JSON.stringify(),
        headers: {
          "Content-Type": "application/json",
